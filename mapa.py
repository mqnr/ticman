def nuevo(*parejas):
    ...
    :param *parejas: una cantidad variable de tuplas, donde cada tupla representa una pareja clave-valor
...
def obtener(mapa, clave):
    ...
    :returns: el valor asociado con la clave dada, o None si la clave no se encuentra en el mapa
    """
    for pareja in mapa:
        if pareja[0] == clave:
            return pareja[1]
...
def actualizar(mapa, pareja):
    """
    Actualizar una pareja clave-valor en el pseudo-mapa. Si la clave no existe, añadir la pareja.
...
def remover(mapa, clave):
    ...
    :returns: True si la pareja fue removida, False si la clave no se encontró en el mapa
    """
    for pareja in mapa:
        if pareja[0] == clave:
            mapa.remove(pareja)
